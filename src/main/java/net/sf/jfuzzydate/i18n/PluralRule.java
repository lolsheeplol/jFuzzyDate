...
public enum PluralRule {
	...
	RULE5 {
		public int selectPluralForm(final int number) {
			...
			if (absNumber == 1) {
				pluralForm = 0;
			} else if ((absNumber == 0) || endsBetween(absNumber, 1, 19, 2)) {
				pluralForm = 1;
			} else {
				pluralForm = 2;
			}
	 ...
	 * @param digits
	 *            the number of digits to be checked.
	 * 
	 ...
	 *             unknown or not implemented plural rule.
	 */
	public abstract int selectPluralForm(final int number);
}
