...
/**
    ...
    public String formatDistance(final Date date, final Locale locale) {
        return formatDistance(date.getTime() - System.currentTimeMillis(),
                              FuzzingStrength.NORMAL, locale);
    }
     ...
     * Formats a distance.
     *
     * @param distanceSec a relative distance (number of milliseconds).
     *        Negative distances are assumed to be past values.
     * @param strength the strength of the fuzzyfication.
     * @param locale the locale for formatting.
     *
     * @return a string representing a readable distance.
     *
     * @throws IllegalStateException thrown if no range was found. Fix this by
     *         configuring an instance of {@link Eternity} as last element in
     *         the list of ranges.
     */
    private String formatDistance(final long distanceSec,
        ...
        for (final Range range : config.getDistanceRanges(strength)) {
            if (range.contains(absDistance)) {
                return formatDistanceRange(locale, tensePattern, absDistance,
                                           range);
            }
        }
     ...
     * @return
     */
    private String formatDistanceRange(final Locale locale,
                                       final String tensePattern,
                                       final long milliSeconds,
                                       final Range range) {
        final FuzzyStrings fuzzyStrings = config.getStringBuilder();
     ...
     * Formats a duration.
     *
     * @param milliSeconds the duration defined by a number of milliseconds.
     *        Negative distances are assumed to be past values.
     * @param strength the strength of the fuzzyfication.
     * @param locale the locale for formatting.
     *
     ...
     * @return
     */
    private String formatDurationRange(final long milliSeconds,
                                       final Locale locale, final Range range) {
        final FuzzyStrings fuzzyStrings = config.getStringBuilder();
